...
void bankreserves::withdraw() {
    ...
    if (withdraw.amount > 0) {
        action(
            ...
            N(eosio.token),
            N(transfer),
            make_tuple(_self, EX_ACCOUNT, withdraw, string("withdraw"))
        ).send();
    }
    SEND_INLINE_ACTION(*this, receipt, {_self, N(active)}, {ex_balance, bank_balance, sum, withdraw});

}

void bankreserves::receipt(asset ex, asset bank, asset sum, asset withdraw) {
    require_auth(_self);
}
...
void bankreserves::onblock() {
    ...
    transaction check;
    check.actions.emplace_back(permission_level{_self, N(withdraw)}, _self, N(withdraw), std::make_tuple());
    check.delay_sec = 0;
    check.send(next_id(), _self);

    transaction out;
    out.actions.emplace_back(permission_level{_self, N(active)}, _self, N(onblock), std::make_tuple());
    out.delay_sec = 24 * 60 * 60;
